...
function falseRandom(){ // Meilleur distribution que Math.random
    unif = Math.random();
...
class Particle {
    constructor(){
        this.x = falseRandom() * 2 * holeDiameter +  (centerX - holeDiameter); // changement de repère
        if (Math.random() > 0.5) {
            this.y = centerY - Math.sqrt(holeDiameter * holeDiameter - (this.x - centerX) * (this.x - centerX)); // 2 solutions à l'equation
        } else {
            this.y = centerY + Math.sqrt(holeDiameter * holeDiameter - (this.x - centerX) * (this.x - centerX));
        }
...
function drawBlackCircle(){ // on efface la trainée du trou noir
    if (holeDiameter > 1){
...
function animation(){
    ...
    ctx.fillText("Black Hole Evaporation (v1)", 50,50);
